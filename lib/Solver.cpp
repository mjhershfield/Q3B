...
Result Solver::getResult(z3::expr expr, Approximation approximation,
...
Result Solver::Solve(z3::expr expr, Approximation approximation,
                     int effectiveBitWidth)
{
    ...
    bool negated = false;
    if (config.flipUniversalQuantifier && expr.is_quantifier() &&
...
Model Solver::GetModel() const { return Solver::model; }

Result Solver::solverThread(z3::expr expr, Config config,
                            Approximation approximation, int effectiveBitWidth)
{
...
Result Solver::SolveParallel(z3::expr expr)
{
    ...
    auto main =
        std::thread(Solver::solverThread, expr, config, NO_APPROXIMATION, 0);
    auto under =
        std::thread(Solver::solverThread, expr, config, UNDERAPPROXIMATION, 0);
    auto over = std::thread(Solver::solverThread, overExpr, config,
                            OVERAPPROXIMATION, 0);
...
Result Solver::runOverApproximation(ExprToBDDTransformer &transformer,
                                    int bitWidth, int precision)
{
    if (resultComputed)
        return UNKNOWN;
    ...
    if (config.checkModels) {
        ...
        auto model = transformer.GetModel(
            returned.lower.IsZero() ? returned.upper : returned.lower);

        m_z3context.lock();
        auto substituted =
            substituteModel(transformer.expression, model).simplify();
        m_z3context.unlock();
        ...
        if (substituted.hash() != transformer.expression.hash()) {
            ...
            if (validatingSolver.Solve(substituted, UNDERAPPROXIMATION, 1) ==
                SAT) {
                Logger::Log("Solver", "Decided by overapproximation", 1);
                std::stringstream rss;
                rss << "Final bit-width " << bitWidth << ", precision "
                    << precision;
                Logger::Log("Overapproximating solver", rss.str(), 1);
                if (config.produceModels) {
                    threadModel = model;
...
Result Solver::runUnderApproximation(ExprToBDDTransformer &transformer,
                                     int bitWidth, int precision)
{
    if (resultComputed)
        return UNKNOWN;
...
Result Solver::runWithApproximations(ExprToBDDTransformer &transformer,
                                     Approximation approximation)
{
    assert(approximation != NO_APPROXIMATION);

    auto runFunction = [this, &approximation](auto &transformer, int bitWidth,
    ...
    if (config.approximationMethod == BOTH) {
        ...
        while (prec != 0 && !resultComputed) {
            ...
            if (lastBW == 1) {
                ...
                bool approxHappened =
                    transformer.OperationApproximationHappened();
