...
ExprToBDDTransformer::ExprToBDDTransformer(z3::context &ctx, z3::expr e,
                                           Config config)
...
void ExprToBDDTransformer::getVars(const z3::expr &e)
{
    ...
    if (e.is_const() && !e.is_numeral()) {
        ...
        for (int i = 0; i < boundVariables; i++) {
            ...
            std::unique_lock<std::mutex> lk(Solver::m_z3context);
            var c =
                make_pair(current_symbol.str(),
                          current_sort.is_bool() ? 1 : current_sort.bv_size());
...
void ExprToBDDTransformer::loadVars()
{
    ...
    for (auto const &group : orderedGroups) {
        ...
        for (auto const &v : group) {
            int bitnum = v.second;
            Bvec varBvec =
                Bvec::bvec_var(bddManager, bitnum, offset + i, group.size());
...
BDDInterval
ExprToBDDTransformer::getConjunctionBdd(const vector<expr> &arguments,
                                        const vector<boundVar> &boundVars,
                                        bool onlyExistentials, bool isPositive)
{
    return getConnectiveBdd(
        arguments, boundVars, onlyExistentials, isPositive,
...
BDDInterval
ExprToBDDTransformer::getDisjunctionBdd(const vector<expr> &arguments,
                                        const vector<boundVar> &boundVars,
                                        bool onlyExistentials, bool isPositive)
{
    return getConnectiveBdd(
...
bool ExprToBDDTransformer::correctBoundVars(
    const std::vector<boundVar> &boundVars,
    const std::vector<boundVar> &cachedBoundVars) const
{
    ...
    for (int i = 0; i < pairsCount; i++) {
        if (cachedBoundVars[cachedBoundVars.size() - i - 1] !=
            boundVars[boundVars.size() - i - 1]) {
            return false;
        }
    }
...
BDDInterval
ExprToBDDTransformer::getBDDFromExpr(const expr &e,
                                     const vector<boundVar> &boundVars,
                                     bool onlyExistentials, bool isPositive)
{
    assert(e.is_bool());
    ...
    if (e.is_var()) {
        ...
        return BDDInterval{
            (vars.at(bVar.first) == Bvec::bvec_true(bddManager, 1))
        ...
        return insertIntoCaches(
            e,
            BDDInterval{(vars.at(exprString) == Bvec::bvec_true(bddManager, 1))
                            .GetBDD(bddManager.bddZero())},
            boundVars, isPositive);
    } else if (e.is_app()) {
        func_decl f = e.decl();
        unsigned num = e.num_args();
        ...
        if (decl_kind == Z3_OP_EQ) {
            ...
            if (sort.is_bv()) {
                ...
                if (config.approximationMethod == OPERATIONS ||
                    ...
                    result = BDDInterval(
                        Bvec::bvec_equ(
                            getBvecFromExpr(e.arg(0), boundVars).value,
                            getBvecFromExpr(e.arg(1), boundVars).value)
                            .GetBDD(bddManager.bddZero()));
                }
            } else if (sort.is_bool()) {
                result = getBDDFromExpr(e.arg(0), boundVars, false, isPositive)
                             .Xnor(getBDDFromExpr(e.arg(1), boundVars, false,
                                                  isPositive));
            }

            return insertIntoCaches(e, result, boundVars, isPositive);
        } else if (decl_kind == Z3_OP_NOT) {
            return !getBDDFromExpr(e.arg(0), boundVars, onlyExistentials,
                                   !isPositive);
            ...
            auto result = getConjunctionBdd(arguments, boundVars,
                                            onlyExistentials, isPositive);
            return insertIntoCaches(e, result, boundVars, isPositive);
            ...
            auto result = getDisjunctionBdd(arguments, boundVars,
                                            onlyExistentials, isPositive);
            return insertIntoCaches(e, result, boundVars, isPositive);
        } else if (decl_kind == Z3_OP_IMPLIES) {
            checkNumberOfArguments<2>(e);

            auto result = !getBDDFromExpr(e.arg(0), boundVars, onlyExistentials,
                                          !isPositive) +
                          getBDDFromExpr(e.arg(1), boundVars, onlyExistentials,
                                         isPositive);
            return insertIntoCaches(e, result, boundVars, isPositive);
            ...
            return insertIntoCaches(e, bvec_ule(arg0, arg1, isPositive),
                                    boundVars, isPositive);
            ...
            return insertIntoCaches(e, bvec_ult(arg0, arg1, isPositive),
                                    boundVars, isPositive);
            ...
            return insertIntoCaches(e, bvec_ule(arg1, arg0, isPositive),
                                    boundVars, isPositive);
            ...
            return insertIntoCaches(e, bvec_ult(arg1, arg0, isPositive),
                                    boundVars, isPositive);
            ...
            if (config.approximationMethod == OPERATIONS ||
                ...
                leastNumber.set(arg0.bitnum() - 1,
                                MaybeBDD(bddManager.bddOne()));

                Bvec greatestNumber =
                    Bvec::bvec_true(bddManager, arg0.bitnum());
                greatestNumber.set(arg0.bitnum() - 1,
                                   MaybeBDD(bddManager.bddZero()));
                ...
                return isPositive ? BDDInterval(over, under)
                                  : BDDInterval(under, over);
            } else {
                result =
                    Bvec::bvec_slte(arg0, arg1).GetBDD(bddManager.bddZero());
            }
            ...
            if (config.approximationMethod == OPERATIONS ||
                ...
                result =
                    Bvec::bvec_slth(arg0, arg1).GetBDD(bddManager.bddZero());
            }
            ...
            auto arg0 = getBDDFromExpr(e.arg(0), boundVars, onlyExistentials,
                                       isPositive);
            auto arg1 = getBDDFromExpr(e.arg(1), boundVars, onlyExistentials,
                                       isPositive);
            auto arg2 = getBDDFromExpr(e.arg(2), boundVars, onlyExistentials,
                                       isPositive);
        ...
        for (int i = 0; i < boundVariables; i++) {
            ...
            BoundType bt = Z3_is_quantifier_forall(*context, ast) ? UNIVERSAL
                                                                  : EXISTENTIAL;

            std::unique_lock<std::mutex> lk(Solver::m_z3context);
            newBoundVars.push_back(
                std::pair<string, BoundType>(current_symbol.str(), bt));
        }
...
Approximated<Bvec> ExprToBDDTransformer::getApproximatedVariable(
    const std::string &varName, int newBitWidth, const ApproximationType &at)
{
...
Approximated<Bvec>
ExprToBDDTransformer::getBvecFromExpr(const expr &e,
                                      const vector<boundVar> &boundVars)
{
    assert(e.is_bv());
    ...
    for (const auto &cache : caches) {
        ...
        if (item != cache.end() &&
            correctBoundVars(boundVars, (item->second).second)) {
            return (item->second).first;
        }
    }
    ...
    if (e.is_var()) {
        ...
        if ((config.approximationMethod == VARIABLES ||
             config.approximationMethod == BOTH) &&
            ((bVar.second == EXISTENTIAL &&
              approximation == UNDERAPPROXIMATION) ||
             (bVar.second == UNIVERSAL &&
              approximation == OVERAPPROXIMATION))) {
            return getApproximatedVariable(bVar.first, variableBitWidth,
                                           approximationType);
        }
        ...
        if ((config.approximationMethod == VARIABLES ||
            ...
            approximation == UNDERAPPROXIMATION) {
            std::unique_lock<std::mutex> lk(Solver::m_z3context);
            auto result = getApproximatedVariable(
                e.to_string(), variableBitWidth, approximationType);
            return insertIntoCaches(e, result, boundVars);
        }

        std::unique_lock<std::mutex> lk(Solver::m_z3context);
        return insertIntoCaches(e, {vars.at(e.to_string()), PRECISE},
                                boundVars);
    } else if (e.is_app()) {
        func_decl f = e.decl();
        unsigned num = e.num_args();
        ...
        if (decl_kind == Z3_OP_BADD) {
            if ((config.approximationMethod == OPERATIONS ||
                ...
                return bvec_assocOp(
                    e,
                    std::bind(Bvec::bvec_add_nodeLimit, _1, _2,
                              precisionMultiplier * operationPrecision),
                    boundVars);
            }
            ...
            if (e.arg(1).is_numeral()) {
                return bvec_unOp(
                    e, [&](auto x) { return x << getNumeralValue(e.arg(1)); },
                    boundVars);
            } else {
                return bvec_binOp(
                    e, [](auto x, auto y) { return x << y; }, boundVars);
            ...
            if (e.arg(1).is_numeral()) {
                return bvec_unOp(
                    e,
                    [&](auto x) {
                        return x.bvec_shrfixed(getNumeralValue(e.arg(1)),
                                               x[bitwidth - 1]);
                    },
                    boundVars);
            } else {
                return bvec_binOp(
                    e,
                    [&](auto x, auto y) {
                        return Bvec::bvec_shr(x, y, x[bitwidth - 1]);
                    },
                    boundVars);
            }
            ...
            for (int i = num - 1; i >= 0; i--) {
                auto [arg, argOpPrecision, argVarPrecision] =
                    getBvecFromExpr(e.arg(i), boundVars);
                currentBvec = Bvec::bvec_map2(
                    currentBvec, arg.bvec_coerce(newSize) << offset,
                    [&](const MaybeBDD &a, const MaybeBDD &b) {
                        return a ^ b;
                    });
            ...
            return insertIntoCaches(e, {currentBvec, opPrecision, varPrecision},
                                    boundVars);
            ...
            return bvec_assocOp(
                e, [&](const Bvec &a, const Bvec &b) { return a | b; },
                boundVars);
        ...
        } else if (decl_kind == Z3_OP_BUREM || decl_kind == Z3_OP_BUREM_I ||
                   decl_kind == Z3_OP_BUDIV || decl_kind == Z3_OP_BUDIV_I) {
            checkNumberOfArguments<2>(e);

            Bvec div = Bvec::bvec_false(bddManager, e.decl().range().bv_size());
            Bvec rem = Bvec::bvec_false(bddManager, e.decl().range().bv_size());

            auto [arg0, arg0OpPrecision, arg0VarPrecision] =
                getBvecFromExpr(e.arg(0), boundVars);
            auto [arg1, arg1OpPrecision, arg1VarPrecision] =
            ...
            if (e.arg(1).is_numeral() && e.get_sort().bv_size() <= 32) {
                result =
                    arg0.bvec_divfixed(getNumeralValue(e.arg(1)), div, rem);
            } else if ((config.approximationMethod == OPERATIONS ||
                        config.approximationMethod == BOTH) &&
                       operationPrecision != 0) {
                result = Bvec::bvec_div_nodeLimit(arg0, arg1, div, rem,
                                                  precisionMultiplier *
                                                      operationPrecision);
            } else {
            ...
            expr e = ite(msb_s == zero && msb_t == zero, udiv(arg0, arg1),
                         ite(msb_s == one && msb_t == zero, -udiv(-arg0, arg1),
                             ite(msb_s == zero && msb_t == one,
                                 -udiv(arg0, -arg1), udiv(-arg0, -arg1))));
            ...
            expr e = ite(msb_s == zero && msb_t == zero, urem(arg0, arg1),
                         ite(msb_s == one && msb_t == zero, -urem(-arg0, arg1),
                             ite(msb_s == zero && msb_t == one,
                                 urem(arg0, -arg1), -urem(-arg0, -arg1))));
            clearCaches();
            ...
            if (arg0.upper != arg0.lower) {
                auto unknown =
                    Bvec{bddManager, e.get_sort().bv_size(), MaybeBDD{}};
                return insertIntoCaches(
                    e, {unknown, APPROXIMATED, APPROXIMATED}, boundVars);
            }
            ...
            return insertIntoCaches(e, {result, APPROXIMATED, APPROXIMATED},
                                    boundVars);
        } else {
...
BDD ExprToBDDTransformer::Proccess()
{
    ...
    if (expression.is_app() && expression.decl().decl_kind() == Z3_OP_TRUE) {
        return bddManager.bddOne();
               ...
               expression.decl().decl_kind() == Z3_OP_FALSE) {
...
BDDInterval
ExprToBDDTransformer::ProcessUnderapproximation(int bitWidth,
...
BDDInterval
ExprToBDDTransformer::ProcessOverapproximation(int bitWidth,
                                               unsigned int precision)
{
    approximation = OVERAPPROXIMATION;
    variableBitWidth = bitWidth;
...
Bvec ExprToBDDTransformer::bvec_mul(Bvec &arg0, Bvec &arg1)
{
    ...
    if (config.approximationMethod == OPERATIONS ||
        config.approximationMethod == BOTH) {
        return Bvec::bvec_mul_nodeLimit(
                   arg0, arg1, precisionMultiplier * operationPrecision)
            .bvec_coerce(bitNum);
    }
...
BDDInterval ExprToBDDTransformer::bvec_ule(Bvec &arg0, Bvec &arg1,
                                           bool isPositive)
{
    if (config.approximationMethod == OPERATIONS ||
        config.approximationMethod == BOTH) {
        ...
        auto under = Bvec::bvec_lte_underApprox(arg0, arg1) |
                     Bvec::bvec_equ_underApprox(
                         arg0, Bvec::bvec_false(bddManager, arg0.bitnum())) |
                     Bvec::bvec_equ_underApprox(
                         arg1, Bvec::bvec_true(bddManager, arg1.bitnum()));
...
BDDInterval ExprToBDDTransformer::bvec_ult(Bvec &arg0, Bvec &arg1,
                                           bool isPositive)
{
    BDDInterval result;
    ...
    if (config.approximationMethod == OPERATIONS ||
        ...
        auto over = Bvec::bvec_lth_overApprox(arg0, arg1) &
                    Bvec::bvec_nequ_overApprox(
                        arg0, Bvec::bvec_true(bddManager, arg0.bitnum())) &
                    Bvec::bvec_nequ_overApprox(
                        arg1, Bvec::bvec_false(bddManager, arg1.bitnum()));
...
Approximated<Bvec>
ExprToBDDTransformer::bvec_assocOp(const z3::expr &e,
                                   const std::function<Bvec(Bvec, Bvec)> &op,
                                   const std::vector<boundVar> &boundVars)
{
    unsigned num = e.num_args();
    ...
    for (unsigned int i = 1; i < num; i++) {
        toReturn =
            toReturn.Apply2<Bvec>(getBvecFromExpr(e.arg(i), boundVars), op);
    }
...
Approximated<Bvec>
ExprToBDDTransformer::bvec_binOp(const z3::expr &e,
                                 const std::function<Bvec(Bvec, Bvec)> &op,
                                 const std::vector<boundVar> &boundVars)
{
    auto result = getBvecFromExpr(e.arg(0), boundVars)
                      .Apply2<Bvec>(getBvecFromExpr(e.arg(1), boundVars), op);

    return insertIntoCaches(e, result, boundVars);
}

Approximated<Bvec>
ExprToBDDTransformer::bvec_unOp(const z3::expr &e,
                                const std::function<Bvec(Bvec)> &op,
                                const std::vector<boundVar> &boundVars)
{
...
Model ExprToBDDTransformer::GetModel(BDD modelBdd)
{
    ...
    for (const auto &[name, bw] : constSet) {
        ...
        const auto &[varBvec, _opPrecise, _varPrecise] =
            getApproximatedVariable(name, variableBitWidth, approximationType);
        for (int i = 0; i < bw; i++) {
            if ((modelBdd & !varBvec[i].GetBDD(bddManager.bddZero()))
                    .IsZero()) {
                modelBV[bw - i - 1] = true;
...
void ExprToBDDTransformer::PrintNecessaryVarValues(BDD bdd,
                                                   const std::string &varName)
{
    ...
    for (unsigned i = 0; i < bvec.bitnum(); i++) {
        if ((bdd & !bvec[i].GetBDD(bddManager.bddZero())).IsZero()) {
            ...
            newVal = true;
        } else if (i != bvec.bitnum() - 1 &&
                   (bdd &
                    (bvec[bvec.bitnum() - 1].GetBDD(bddManager.bddZero()) ^
                     bvec[i].GetBDD(bddManager.bddZero())))
                       .IsZero()) {
...
Approximated<Bvec>
ExprToBDDTransformer::insertIntoCaches(const z3::expr &expr,
                                       const Approximated<Bvec> &bvec,
                                       const std::vector<boundVar> &boundVars)
{
...
BDDInterval ExprToBDDTransformer::insertIntoCaches(
    const z3::expr &expr, const BDDInterval &bdd,
    const std::vector<boundVar> &boundVars, bool isPositive)
{
    bddExprCache.insert({{(Z3_ast)expr, isPositive}, {bdd, boundVars}});

    if (bdd.upper == bdd.lower) {
        sameBWPreciseBdds.insert(
            {{(Z3_ast)expr, isPositive}, {bdd, boundVars}});
    }
...
{
    return std::all_of(bvec.m_bitvec.begin(), bvec.m_bitvec.begin(),
                       [](auto &bit) { return bit.IsOne(); });
}
...
bool ExprToBDDTransformer::isInterrupted() { return Solver::resultComputed; }
