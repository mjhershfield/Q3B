    ...
    expr e, bool isPositive, Goal goal = NONE)
{
    ...
    if (e.is_const() && !e.is_numeral()) {
        ...
        if (num != 0) {
            ...
            if (decl_kind == Z3_OP_NOT) {
                return countVariableOccurences(e.arg(0), !isPositive);
                       ...
                       (decl_kind == Z3_OP_EQ && e.arg(0).is_bool())) {
                addCounts(countVariableOccurences(e.arg(0), isPositive),
                          varCounts);
                addCounts(countVariableOccurences(e.arg(1), isPositive),
                          varCounts);
                ...
                // counts(ite(a, b, c)) = 2*counts(a) + max(counts(b),
                // counts(c))
                varCounts = countVariableOccurences(e.arg(1), isPositive);
                maxCounts(countVariableOccurences(e.arg(2), isPositive),
                          varCounts);
            ...
            if (decl_kind == Z3_OP_ULEQ || decl_kind == Z3_OP_ULT) {
                addCounts(
                    countVariableOccurences(e.arg(0), isPositive, UNSIGN_MIN),
                    varCounts);
                addCounts(
                    countVariableOccurences(e.arg(1), isPositive, UNSIGN_MAX),
                    varCounts);
                return varCounts;
            }
            ...
            if (decl_kind == Z3_OP_UGEQ || decl_kind == Z3_OP_UGT) {
                addCounts(
                    countVariableOccurences(e.arg(0), isPositive, UNSIGN_MAX),
                    varCounts);
                addCounts(
                    countVariableOccurences(e.arg(1), isPositive, UNSIGN_MIN),
                    varCounts);
                return varCounts;
            }
            ...
            if (decl_kind == Z3_OP_SLEQ || decl_kind == Z3_OP_SLT) {
                addCounts(
                    countVariableOccurences(e.arg(0), isPositive, SIGN_MIN),
                    varCounts);
                addCounts(
                    countVariableOccurences(e.arg(1), isPositive, SIGN_MAX),
                    varCounts);
                return varCounts;
            }
            ...
            if (decl_kind == Z3_OP_SGEQ || decl_kind == Z3_OP_SGT) {
                addCounts(
                    countVariableOccurences(e.arg(0), isPositive, SIGN_MAX),
                    varCounts);
                addCounts(
                    countVariableOccurences(e.arg(1), isPositive, SIGN_MIN),
                    varCounts);
                return varCounts;
            }

            for (unsigned i = 0; i < num; i++) {
                addCounts(countVariableOccurences(e.arg(i), isPositive),
                          varCounts);
            }
        } else if (f.name() != NULL) {
            z3::sort s = f.range();
        ...
        if (e.get_sort().is_bv() && !e.is_numeral()) {
            ...
            subformulaVariableCounts.insert(
                {{e, !isPositive, goal}, varCounts});
        }
        return varCounts;
        ...
        for (int i = numBound - 1; i >= 0; i--) {
            ...
            if (current_sort.is_bool()) {
                boundVars.push_back(
                    context->bool_const(current_symbol.str().c_str()));
            } else // is BV
            {
                boundVars.push_back(context->bv_const(
                    current_symbol.str().c_str(), current_sort.bv_size()));
            }
        }

        auto result = countVariableOccurences(
            e.body().substitute(boundVars).simplify(), isPositive);
...
void UnconstrainedVariableSimplifier::SimplifyIte()
{
    ...
    bool anyUnconstrained =
        std::any_of(variableCounts.begin(), variableCounts.end(),
                    [](const auto &var) { return var.second == 1; });
    if (!anyUnconstrained) {
        return;
    }
...
z3::expr UnconstrainedVariableSimplifier::simplifyOnce(
    expr e, std::vector<BoundVar> boundVars, bool isPositive = true,
    Goal goal = NONE)
{
    ...
    if (((isPositive && item != trueSimplificationCache.end()) ||
         (!isPositive && item != falseSimplificationCache.end()))) {
        ...
        for (int i = 0; i < pairsCount; i++) {
            string oldVarName =
                std::get<0>(cachedBoundVars[cachedBoundVars.size() - i - 1]);
            string newVarName =
                std::get<0>(boundVars[boundVars.size() - i - 1]);
    ...
    if (e.is_app()) {
        ...
        if (decl_kind == Z3_OP_BADD || decl_kind == Z3_OP_BXOR) {
            for (unsigned int i = 0; i < num; i++) {
                ...
                for (unsigned int j = 0; j < num; j++) {
                    if (i != j &&
                        !isBefore(e.arg(j), e.arg(i), boundVars, isPositive)) {
                        allBefore = false;
                        break;
                    }
                }
            ...
            if (unconstrained0 && unconstrained1 &&
                getBoundType(e.arg(0), boundVars) ==
                    getBoundType(e.arg(1), boundVars)) {
            ...
            if (unconstrained0 && unconstrained1 &&
                ...
                if (isBefore(e.arg(0), e.arg(1), boundVars, isPositive)) {
                    ...
                    return e.arg(0);
                       ...
                       isBefore(e.arg(1), e.arg(0), boundVars, isPositive)) {
                return e.arg(0) & (e.arg(1) | -e.arg(1));
            } else if (unconstrained1 &&
                       isBefore(e.arg(0), e.arg(1), boundVars, isPositive)) {
            ...
            if (unconstrained0 && unconstrained1 &&
                ...
                if (goalUnconstrained) {
                    bool flip =
                        (!isPositive &&
                         getBoundType(e.arg(0), boundVars) == EXISTENTIAL) ||
                        (isPositive &&
                         getBoundType(e.arg(0), boundVars) == UNIVERSAL);

                    if ((!flip && goal == SIGN_MAX) ||
                        (flip && goal == SIGN_MIN)) {
                        auto maxSigned =
                            concat(context->bv_val(0, 1),
                                   context->bv_val(-1, bvSize - 1)); // 011...1
                        return z3::shl(z3::ashr(maxSigned, arg1), arg1);
                    } else if ((!flip && goal == UNSIGN_MAX) ||
                               (flip && goal == UNSIGN_MIN)) {
                        return z3::shl(ones, e.arg(1));
                    } else if ((!flip && goal == SIGN_MIN) ||
                               (flip && goal == SIGN_MAX)) {
                        auto minSigned = z3::concat(
                            context->bv_val(1, 1),
                            context->bv_val(0, bvSize - 1)); // 1000...0
                        return z3::ite(z3::ult(arg1, bvSize), minSigned,
                                       zeroes);
                    } else if ((!flip && goal == UNSIGN_MIN) ||
                               (flip && goal == UNSIGN_MAX)) {
                        return zeroes;
                    }
                }
                ...
                auto shiftExpr =
                    to_expr(*context,
                            Z3_mk_bvshl(*context, (Z3_ast)ones, (Z3_ast)arg1));
                ...
                if (goalUnconstrained) {
                    ...
                    bool flip =
                        (!isPositive &&
                         getBoundType(e.arg(1), boundVars) == EXISTENTIAL) ||
                        (isPositive &&
                         getBoundType(e.arg(1), boundVars) == UNIVERSAL);

                    if ((!flip && goal == UNSIGN_MIN) ||
                        (flip && goal == UNSIGN_MAX)) {
                        return zeroes;
                    } else if ((!flip && goal == SIGN_MIN) ||
                               (flip && goal == SIGN_MAX)) {
            ...
            if (unconstrained0 && unconstrained1 &&
                ...
                if (goalUnconstrained) {
                    ...
                    if ((!flip && goal == SIGN_MAX) ||
                        ...
                        auto maxSigned =
                            concat(context->bv_val(0, 1),
                                   context->bv_val(-1, bvSize - 1));
                        return z3::ite(
                            arg1 == 0, maxSigned,
                            to_expr(*context,
                                    Z3_mk_bvlshr(*context, ones, arg1)));
                    } else if ((!flip && goal == UNSIGN_MAX) ||
                               (flip && goal == UNSIGN_MIN)) {
                ...
                auto shiftExpr =
                    to_expr(*context,
                            Z3_mk_bvlshr(*context, (Z3_ast)ones, (Z3_ast)arg1));
                return (e.arg(0) & shiftExpr);
            } else if (unconstrained1 &&
                       isBefore(e.arg(0), e.arg(1), boundVars, isPositive)) {
                ...
                if (goalUnconstrained) {
                    ...
                    if ((!flip && goal == SIGN_MAX) ||
                        (flip && goal == SIGN_MIN)) {
                        return z3::ite(arg0.extract(bvSize - 1, bvSize - 1) ==
                                           0,
                                       arg0, f(arg0, 1));
                    } else if ((!flip && goal == UNSIGN_MAX) ||
                               (flip && goal == UNSIGN_MIN)) {
            ...
            if (unconstrained0 && unconstrained1 &&
                    ...
                    if ((!flip && goal == SIGN_MAX) ||
                        ...
                        auto maxSigned =
                            concat(context->bv_val(0, 1),
                                   context->bv_val(-1, bvSize - 1));
                        return to_expr(*context,
                                       Z3_mk_bvashr(*context, maxSigned, arg1));
                    } else if ((!flip && goal == UNSIGN_MAX) ||
                               (flip && goal == UNSIGN_MIN)) {
                        return ones;
                    } else if ((!flip && goal == SIGN_MIN) ||
                ...
                // can return precisely numbers with the same t+1
                // most-significant bits

                // (bvashr u t) -> (ite (or (= (bvand u (bvashr minSigned t)) 0)
                //                          (= (bvand u (bvashr minSigned t))
                //                          (bvashr minSigned t)))
                //                      u
                //                      0)
                auto onlyTopFromU = e.arg(0) & f(minSigned, e.arg(1));
                return z3::ite(onlyTopFromU == 0 ||
                                   onlyTopFromU == f(minSigned, e.arg(1)),
                               e.arg(0), zeroes);
            } else if (unconstrained1 &&
                       isBefore(e.arg(0), e.arg(1), boundVars, isPositive)) {
                int bvSize = e.arg(0).get_sort().bv_size();
            ...
            if (unconstrained0 && unconstrained1 &&
                ...
                auto bvult =
                    to_expr(*context, Z3_mk_bvult(*context, (Z3_ast)e.arg(0),
                                                  (Z3_ast)e.arg(1)));

                // bvurem u t may return all values from {0,...,t-1}
                // (bvurem u t) -> (ite (= t 0) u (ite (bvult u t) u 0))
                return ite(e.arg(1) == zero, e.arg(0),
                ...
                if (goalUnconstrained) {
                    ...
                    if ((!flip && goal == UNSIGN_MIN) ||
                        ...
                        return e.arg(0); // by setting divisor to zero
                               ...
                               (flip && goal == SIGN_MAX)) {
                        // unsigned division can not change positive sign to
                        // negative sign if positive, reduce to zero if
                        // negative, leave the same, because remainder will not
                        // decrease the negative number
                        return z3::ite(
                            e.arg(0).extract(bvSize - 1, bvSize - 1) == 0, zero,
                            e.arg(0));
                    }
                }

                // bvurem t u may return all values from {0,...,(t-1)/2} \cup
                // {t} (bvurem t u) -> (ite (= t 0) 0 ((ite (bvule u (bvudiv (-
                // t 1) 2)) u t)))
                auto bvule = to_expr(
                    *context,
                    Z3_mk_bvule(
            ...
            if (unconstrained1 &&
                isBefore(e.arg(0), e.arg(1), boundVars, isPositive) &&
                goalUnconstrained) {
                bool flip = (!isPositive && getBoundType(e.arg(1), boundVars) ==
                                                EXISTENTIAL) ||
                            (isPositive &&
                             getBoundType(e.arg(1), boundVars) == UNIVERSAL);
                ...
                if ((!flip && goal == UNSIGN_MAX) ||
                    (flip && goal == UNSIGN_MIN)) {
                    return ones;
                } else if ((!flip && goal == UNSIGN_MIN) ||
                           (flip && goal == UNSIGN_MAX)) {
                    return z3::ite(arg0 == ones, context->bv_val(1, bvSize),
                                   zeroes);
                } else if ((!flip && goal == SIGN_MAX) ||
                           (flip && goal == SIGN_MIN)) {
                    // if positive, do not change, as unsigned division can not
                    // increase the number if negative, divide by 2 to get a
                    // positive number
                    return z3::ite(e.arg(0).extract(bvSize - 1, bvSize - 1) ==
                                       0,
                                   e.arg(0), f(arg0, 2));
                } else if ((!flip && goal == SIGN_MIN) ||
                           (flip && goal == SIGN_MAX)) {
                    // if positive, reduce to -1
                    // if negative, do not change, as division would make it
                    // positive
                    return z3::ite(e.arg(0).extract(bvSize - 1, bvSize - 1) ==
                                       0,
                                   ones, arg0);
            ...
            if (unconstrained0 &&
                ...
                auto bvudiv = to_expr(
                    *context, Z3_mk_bvudiv(*context, (Z3_ast)ones, e.arg(1)));
                auto bvule = to_expr(
                    *context, Z3_mk_bvule(*context, (Z3_ast)e.arg(0), bvudiv));
                ...
                if (goalUnconstrained) {
                    ...
                    if ((!flip && goal == UNSIGN_MIN) ||
                        ...
                        return z3::ite(e.arg(1) == zero, ones, zero);
                               ...
                               (flip && goal == UNSIGN_MIN)) {
                        return z3::ite(e.arg(1) == zero, ones,
                                       bvudiv); // for a/0, the max is 11..1,
                                                // for a/b it is 11...1/b
                    } else if ((!flip && goal == SIGN_MIN) ||
                               (flip && goal == SIGN_MAX)) {
                        auto minSigned = z3::concat(
                            context->bv_val(1, 1),
                ...
                // (bvurem u t) -> (ite (= t 0) (11...1) (ite (bvule u (bvudiv
                // (11...1)/t)) u 0))
                return ite(e.arg(1) == zero, ones, ite(bvule, e.arg(0), zero));
            }
            ...
            for (unsigned int i = 0; i < num; i++) {
                if (isUnconstrained(e.arg(i), boundVars)) {
                    ...
                    if ((boundType == EXISTENTIAL && isPositive) ||
                        (boundType == UNIVERSAL && !isPositive)) {
            ...
            return e;
            //			return ((simplifyOnce(e.arg(0), boundVars,
            //isPositive) || !simplifyOnce(e.arg(1), boundVars, !isPositive)) &&
            //					(!simplifyOnce(e.arg(0),
            //boundVars, !isPositive) || simplifyOnce(e.arg(1), boundVars,
            //isPositive)));
            ...
            if (isUnconstrained(e.arg(0), boundVars) &&
                ...
                if (boundType == EXISTENTIAL) {
                    return isPositive ? context->bool_val(true)
                                      : context->bool_val(false);
                } else {
                    return isPositive ? context->bool_val(false)
                ...
                if (e.arg(0).is_bool()) {
                    ...
                    return simplifyOnce(e.arg(0), boundVars, isPositive) ==
                           simplifyOnce(e.arg(1), boundVars, isPositive);
                }
            }
            ...
            if (isUnconstrained(e.arg(0), boundVars) &&
                ...
                if (boundType == EXISTENTIAL) {
                    ...
                    return isPositive ? e.arg(1) == maxValue
                                      : context->bool_val(true);
                }
            } else if (isUnconstrained(e.arg(1), boundVars) &&
                       isBefore(e.arg(0), e.arg(1), boundVars, isPositive)) {
                ...
                auto minValue = decl_kind == Z3_OP_SLEQ
                                    ? concat(context->bv_val(1, 1),
                                             context->bv_val(0, bvSize - 1))
                ...
                if (boundType == EXISTENTIAL) {
                    ...
                    return isPositive ? e.arg(0) == minValue
                                      : context->bool_val(true);
                }
            }

            auto arg0 =
                simplifyOnce(e.arg(0), boundVars, isPositive,
                             decl_kind == Z3_OP_SLEQ ? SIGN_MIN : UNSIGN_MIN);
            auto arg1 =
                simplifyOnce(e.arg(1), boundVars, isPositive,
                             decl_kind == Z3_OP_SLEQ ? SIGN_MAX : UNSIGN_MAX);
            ...
            if (isUnconstrained(e.arg(0), boundVars) &&
                ...
                if (boundType == EXISTENTIAL) {
                    return isPositive ? !(e.arg(1) == minValue)
                                      : context->bool_val(false);
                } else {
                    return isPositive ? context->bool_val(false)
                                      : !(e.arg(1) == minValue);
                ...
                if (boundType == EXISTENTIAL) {
                    return isPositive ? !(e.arg(0) == maxValue)
                                      : context->bool_val(false);
                } else {
                    return isPositive ? context->bool_val(false)
                                      : !(e.arg(0) == maxValue);
            ...
            auto arg0 =
                simplifyOnce(e.arg(0), boundVars, isPositive,
                             decl_kind == Z3_OP_SLT ? SIGN_MIN : UNSIGN_MIN);
            auto arg1 =
                simplifyOnce(e.arg(1), boundVars, isPositive,
                             decl_kind == Z3_OP_SLT ? SIGN_MAX : UNSIGN_MAX);

            return f(arg0, arg1);
        } else if (decl_kind == Z3_OP_ITE) {
            if (isUnconstrained(e.arg(0), boundVars) &&
                isUnconstrained(e.arg(1), boundVars) &&
                getBoundType(e.arg(0), boundVars) ==
                    getBoundType(e.arg(1), boundVars)) {
                return e.arg(1);
            }
            ...
            if (isUnconstrained(e.arg(0), boundVars) &&
                isUnconstrained(e.arg(2), boundVars) &&
                getBoundType(e.arg(0), boundVars) ==
                    getBoundType(e.arg(2), boundVars)) {
                return e.arg(2);
            }

            auto result =
                ite(e.arg(0), simplifyOnce(e.arg(1), boundVars, isPositive),
                    simplifyOnce(e.arg(2), boundVars, isPositive));
            if (isPositive) {
        ...
        for (unsigned int i = 0; i < num; i++) {
            if (decl_kind == Z3_OP_NOT) {
                arguments.push_back(
                    simplifyOnce(e.arg(i), boundVars, !isPositive));
            } else {
                arguments.push_back(
                    simplifyOnce(e.arg(i), boundVars, isPositive));
            }
        ...
        if (isPositive) {
            boundType = Z3_is_quantifier_forall(*context, ast) ? UNIVERSAL
                                                               : EXISTENTIAL;
        } else {
            boundType = Z3_is_quantifier_forall(*context, ast) ? EXISTENTIAL
                                                               : UNIVERSAL;
        }
        ...
        for (int i = 0; i < numBound; i++) {
            ...
            boundVars.push_back(
                make_tuple(current_symbol.str(), boundType, level));
        }
        ...
        Z3_ast quantAst = Z3_mk_quantifier(
            *context, Z3_is_quantifier_forall(*context, ast),
            Z3_get_quantifier_weight(*context, ast), 0, {}, numBound, sorts,
            decl_names, (Z3_ast)simplifyOnce(e.body(), boundVars, isPositive));
...
bool UnconstrainedVariableSimplifier::isUnconstrained(
    expr e, const vector<BoundVar> &boundVars) const
{
    if (!isVar(e) && e.is_app()) {
        return e.decl().decl_kind() == Z3_OP_EXTRACT &&
               isUnconstrained(e.arg(0), boundVars);
    }

    if (e.is_var()) {
        Z3_ast ast = (Z3_ast)e;
        int deBruijnIndex = Z3_get_index_value(*context, ast);
        return (variableCounts.at(std::get<0>(
                    boundVars.at(boundVars.size() - deBruijnIndex - 1))) == 1);
...
int UnconstrainedVariableSimplifier::getMaxLevel(
    expr e, const std::vector<BoundVar> &boundVars, bool isPositive = true)
{
    ...
    if (e.is_var()) {
        ...
        if (num != 0) {
            for (unsigned i = 0; i < num; i++) {
                auto currentMaxLevel =
                    getMaxLevel(e.arg(i), boundVars, isPositive);
...
bool UnconstrainedVariableSimplifier::isBefore(
    expr a, expr b, const std::vector<BoundVar> &boundVars, bool isPositive)
{
...
BoundType UnconstrainedVariableSimplifier::getBoundType(
    expr e, const std::vector<BoundVar> &boundVars)
{
    if (e.is_var()) {
        Z3_ast ast = (Z3_ast)e;
        int deBruijnIndex = Z3_get_index_value(*context, ast);
...
void UnconstrainedVariableSimplifier::addCounts(
    const std::map<std::string, int> &from, std::map<std::string, int> &to)
{
    for (auto &item : from) {
...
void UnconstrainedVariableSimplifier::maxCounts(
    std::map<std::string, int> &&from, std::map<std::string, int> &to)
{
    for (auto &item : from) {
        auto singleVarCount = to.find(item.first);
        if (singleVarCount == to.end()) {
...
UnconstrainedVariableSimplifier::countFormulaVarOccurences(z3::expr e)
{
